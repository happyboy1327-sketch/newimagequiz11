...
const HUMAN_IMAGE_BLOCKLIST = /(?:^|[\s_./\\-])(?:coin|medal|seal|flag|coat_of_arms|emblem|tomb|temple|grave|map|signature|sabre|poem|feather|quill|pen|symbol|icon|picto|insignia|rank|award|handwriting|drawing|sketch|illustration|calligraphy|landscape|gardenshield|tower)(?:$|[\s_./\\-])|(?:^|[\s_./\\-])(?:청|적|백|황|녹|파|남|흑|blue|red|green|yellow)\d+(?:px)?(?:\.|[\s_.-]|$)/i;
...
async function fillCache() {
    ...
    // 2. 불필요한 하단 섹션 및 TMI/타인 정보 섹션 완전 절단
                           const cutIndex = extractBody.search(/(?:^|\n)\s*={2,}\s*(각주|가족|같이 보기|참고 문헌|참고 자료|경력|학력|기타|외부 링크|[가-힣\s]*작품(?:\s*목록)?|[가-힣\s]*저서|주석|여담|갤러리|가계도|계보)\s*={2,}/i);
