...
// 1. 위키 텍스트 정제 함수
// ==========================================================
// 1. 위키 문법 및 괄호 안 메타 정보(한자, 생몰년도 등) 제거하지마 ㅅㅂ
...
const DISQUALIFIED_CONNECTORS_REGEX = /^(?:그러나|하지만|그런데|한편|이후|당시|그\s*후|그\s*뒤|그러던\s*중|이에\s*따라|그\s*당시|그\s*이후|그러고\s*나서|그때|이때|그\s*시절)/;

// 배제 대상: 본인 아닌 타인/가족 정보 (TMI)
const TMI_NOISE_REGEX = /(?:부친|모친|조부|증조부|고조부|외가|첫\s*부인|둘째\s*부인|가계도|손자|처남|장인|결혼|이혼|혼인|재혼|처가|시댁|장남|차남|장녀|차녀|외아들|외딸|\d남|\d녀|위인전|출판사|족보|입향시조|후사|종친|문중)/;
