...
export function stripMetainfo(text) {
  ...
  // 1) 괄호 내부 정리: 생몰년/연도 및 한자 표기 등은 보존하고 메타 괄호만 삭제
  result = result.replace(/\(([^()]+)\)/g, (match, inner) => {
    // 생몰년/연도(음력 포함)가 포함되어 있으면 보존
    if (/(?:\d{3,4}년|~|음력)/.test(inner)) {
      // 괄호 시작 부분의 불필요한 쉼표 정돈 (예: ", 1876년" -> "1876년")
      const cleaned = inner.replace(/^\s*,\s*/, "").trim();
      return `(${cleaned})`;
    }
    // 자/호/본관 등 순수 메타 정보만 있으면 괄호 통째 삭제
  ...
  result = result
    .replace(/,\s*\(\s*\)/g, "")
    .replace(/\(\s*\)/g, "")
    .replace(/\(\s*,\s*/g, "("); // 괄호 직후 쉼표 정돈

  // 2) 메타 문장 및 절(Clause) 정밀 제거 (자/호/본관 등)
  // 2-1) 독립된 메타 문장 통째 삭제
  result = result.replace(/(?<![가-힣])(?:자|호|본관|시호|아호|별호|태명|아명)\b.*?(?:있다|있었다|전해진다)\.?/g, "");

  // 2-2) 문장 뒤 쉼표(,) 뒤에 붙은 메타 절 삭제 (대괄호 [...] 버그 수정 -> (?:...) 그룹화)
  result = result.replace(/,\s*(?<![가-힣])(?:자|호|본관|시호|아호|별호)\s*[:=는은이]?\s*[^.!?]*(?:본관|시호|아호|별호|호|자|이다|였다)\.?/g, "");
  ...
  result = result
    .replace(/([가-힣]+)으로\s*이다\./g, "$1이다.")                      // '으로이다.' 오염 긴급 복구
    .replace(/(?:,\s*)+,/g, ",")                                      // 다중 쉼표 정돈
    .replace(/,\s*\./g, ".")                                          // 쉼표+마침표 정돈
    .replace(/^\s*,\s*/, "")                                          // 문두 쉼표 제거
    .replace(/\s*\.+\s*(?:\.+\s*)+/g, ".")                            // 연속 마침표(.. , . .) 정돈
    .replace(/([가-힣]+)(?:으로|며|이고|이자|이며)\s*\./g, "$1이다.") // 연결어미 문장 완결 보정
...
const CORE_SIGNIFICANCE_KEYWORDS = [
  "원리", "구조", "기능", "작용", "현상", "이론", "연구", "발견", "규명", "증명", "정립", 
  ...
  "대표", "영향", "의의", "기여", "발전", "역사", "중심", "주요", "핵심", "주요한", "독립운동", 
...
const PASSIVE_BG_REGEX = /(?:(?:지점|시대|무렵|해|곳)이다|위치해\s*있다|일이\s*벌어졌다|상황이었다|태어났다|결혼하였다|결횬했다)/;
...
const REGEX_LEADING_CONNECTORS = /^(?:그러나|하지만|그런데|한편|따라서|게다가|반면|이에|이후|그\s*이후로|결국|그\s*후|또한|그리고|그\s*당시),?\s*/;
...
// TF-IDF 기반 핵심어/문장 중요도 계산
// 각 문장을 하나의 document로 보고,
// 여러 문장에 반복되는 단어는 낮게, 특정 문장에만 등장하는 단어는 높게 평가한다.
function buildTfidfModel(sentences) {
  ...
  const idf = {};
  for (const [token, df] of Object.entries(documentFrequency)) {
    // smoothing: df=0/df=N에서도 안정적으로 계산
    idf[token] = Math.log((documentCount + 1) / (df + 1)) + 1;
  }

  return { tokenized, documentFrequency, idf, documentCount };
}

function getSentenceTfidfScore(tokens, idf) {
  ...
  // 문장 길이에 따른 편향을 줄이기 위해 TF를 토큰 수로 정규화
  let score = 0;
...
function getTopDocumentKeywords(sentences, topK = 10) {
  const { tokenized, idf } = buildTfidfModel(sentences);
  const keywordScores = {};
  ...
  tokenized.forEach((tokens) => {
    ...
    for (const [token, count] of Object.entries(termFrequency)) {
      const tf = count / tokens.length;
      keywordScores[token] = (keywordScores[token] || 0) + tf * (idf[token] || 1);
    }
  });
...
export function buildDescription(
  ...
  // 3) TF-IDF + TextRank 계산
  // 앵커를 포함한 전체 문장을 TF-IDF corpus로 사용한다.
  const analysisSentences = [...anchorSentences, ...candidateSentences];
  const tfidfModel = buildTfidfModel(analysisSentences);
  const topKeywords = getTopDocumentKeywords(analysisSentences, 10);

  const candidateTokenized = candidateSentences.map(tokenize);
  const candidateTfidfScores = candidateTokenized.map((tokens) =>
    getSentenceTfidfScore(tokens, tfidfModel.idf)
  );
  const maxTfidfScore = Math.max(...candidateTfidfScores, 0.001);
...
if (docTitle) {
  ...
  for (const match of subjectMatches) {
    ...
    // TextRank와 TF-IDF를 결합
    // TF-IDF는 '이 문장만 가지고 있는 정보'를 반영하고,
    // TextRank는 전체 문맥에서 중심적인 문장을 반영한다.
    let score = textRankScore * 0.55 + tfidfScore * 0.45;
    ...
    score *= (1 + Math.min(matchCount, 3) * 0.10);
