...
export function stripMetainfo(text) {
  ...
  // 4) 불완전 어미 및 단절 조사 서술어 전환 ㅅㅂ
   // 4) 불완전 어미 및 단절 조사 서술어 전환
  result = result
  // '이었/였/했/되었' + 연결어미(며/고/나/지만 등) → 완전한 서술어로 통합 처리
    .replace(/([가-힣]+?)(이었|였)(?:으며|으나|고|지만|는데)\s*\.?\s*$/g, "$1이었다.")
    .replace(/([가-힣]+?)했(?:으며|으나|고|지만|는데)\s*\.?\s*$/g, "$1했다.")
    .replace(/([가-힣]+?)되었(?:으며|으나|고|지만|는데)\s*\.?\s*$/g, "$1되었다.")
