...
// =========================================================
  ...
  // ---------------------------------------------------------
  // 2-1) 독립된 메타 문장 통째 삭제 (미불 케이스: "자는 원장으로 ... 호가 있다.")
  result = result.replace(/(?<![가-힣])(?:자|호|본관|시호|아호|별호|태명|아명)\b.*?(?:있다|있었다|전해진다)\.?/g, "");

  // 2-2) 문장 뒤 쉼표(,) 뒤에 붙은 메타 절 삭제 및 문장 마감 (김구 케이스: "...정치인으로, 자는 연하, 호는 백범이다." -> "...정치인이다.")
  result = result.replace(/,\s*(?<![가-힣])(?:자|호|본관|시호|아호|별호|태명|)\s*[:=는은이]?\s*[^.!?]*[본관|시호|아호|별호|호|자|이다|였다]\.?/g, "이다.");

  // 2-3) 잔여 단독 메타 구문 제거 (예: "자는 연하이다.")
  result = result.replace(/(?<![가-힣])(?:본관|시호|아호|별호|아명|법명|묘호|호|자)\s*[:=는은이]\s*[^,;.\n]+/g, "");

  // ---------------------------------------------------------
  // 3) 구두점 찌꺼기 정리 및 문장 완결성 어미 보정
  // ---------------------------------------------------------
