const IMPORTANT_KEYWORDS = [
    ...
    "고안", "제작", "개량", "설계", "과학", "기술", "천문", "의학", "수학", "공학", "관측", "발명품", "이론", "법칙", "원리", "측우기", "혼천의", "자격루", "헌신", "보급", "창설", "구제", "지원", "정책", "구호", "봉사", "확산"
];
...
const NUTRITION_REGEX = /(독립|전투|(?:독립|만세|민주화)?운동(?!\s*장)|학설|발명|발견|창시|개혁|통일|건국|재위|집권|해방|혁명|사상|학파|저서|대표작|노벨상|원소|정리|공식|전쟁|함락|전승|수상|발표|설립|창립|개발|발명가|순국|고문|정복|멸망|편입|군현제|도량형|만리장성|분서갱유|황제|칭호|제도|토목|능묘|순행|붕어|개량|설계|과학|기술|천문|의학|수학|공학|헌신|보급|창설|주창|구제|지원|관측|발명품|이론|법칙|원리|측우기|혼천의|자격루|(?!(?:여론|결론|방법론))(?:[가-힣]+론)|(?:[가-힣]+주의))/;
...
function removeMetaBySearch(text) {
    ...
    const metaChainRegex = new RegExp(
        `(?:,\\s*|\\s+)*(?:${singleMeta}(?:,\\s*|\\s+이며|\\s+이었으며|\\s+이고|\\s+)*)+(?:이다|였다|이었다)?`,
        "g"
    );
