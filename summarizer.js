...
export function stripMetainfo(text) {
  ...
  const cleanSentences = rawSentences
    .map((sentence) => {
      ...
      // "호는 ○○이다."
      //
      // 종결어미 앞에서 끊고, 쉼표까지 소비한다.
      const metaClauseRegex =
        /(?:이름|본관|본적|시호|아호|별호|아명|태명|세례명|법명|묘호|당호|호|자|묘|성|씨|휘)(?:\s*\([^)]*\))?\s*(?:는|은)\s+[^,.;!?]*?(?:이며|이고|이자|임|이다|였다|이었다)(?:\s*,)?/g;

      s = s.replace(metaClauseRegex, "");

      // 4) 나열형 "자" / "호" 메타 절
      //
      // "자는 자여(子輿) 또는 자거(子車), 자거(子居), 자전(子展) 등이다."
      // 처럼 쉼표가 여러 번 들어가는 경우를 별도로 처리한다.
      const listedMetaRegex =
        /(?:자|호|아호|별호|시호|본관|본적)(?:\s*\([^)]*\))?\s*(?:는|은)\s+[^.!?]*?(?:등)?\s*(?:이다|이었다|였다|임)\s*(?:,\s*)?/g;

      s = s.replace(listedMetaRegex, "");

      // 5) 인라인 가계 메타 절 제거
      const familyClauseRegex =
        /(?:부친|모친|아버지|어머니|조부|증조부|고조부|외가|장인|처남)(?:은|는)\s+[^,.;!?]*?(?:이며|이고|이자|임|이다|였다|이었다)(?:\s*,)?/g;

      s = s.replace(familyClauseRegex, "");

      // 6) 출생/혈통을 나타내는 종속 절 제거
      //
      // "○○의 아들로 태어나"
      // "○○의 장남으로 태어났다"
      //
      // 단, 뒤에 본문이 이어지는 경우 해당 절만 제거한다.
      const birthClauseRegex =
        /(?:[가-힣A-Za-z0-9·]+(?:의|에서)\s+)?(?:아들|딸|장남|차남|장녀|차녀|막내|후손)(?:으로|로)\s+(?:태어나|출생하[고며])/g;

      s = s.replace(birthClauseRegex, "");

      // 7) 메타 제거 후 구두점 및 공백 정리
      s = s
        .replace(/\s*,\s*,+/g, ",")
        .replace(/,\s*\./g, ".")
        .replace(/^\s*,\s*/, "")
    ...
    .filter((s) => {
      ...
      // 8) 문장 전체가 순수 메타 정보인 경우 제거
      const isPureMetaSentence =
        /^(?:이름|본관|본적|시호|아호|별호|아명|태명|세례명|법명|묘호|당호|호|자|묘|성|씨|휘)(?:\s*\([^)]*\))?\s*(?:는|은)\b/.test(
          s
        );

      if (isPureMetaSentence) return false;

      // 9) 문장 전체가 순수 가계/혈통 TMI인 경우 제거
      const isFamilyTmiSentence =
        /(?:부친|모친|조부|증조부|고조부|외가|어머니|아버지|장남|차남|장녀|차녀|막내|아들|딸|후손)/.test(
          s
        ) &&
        !/(?:전투|저술|개혁|통일|격퇴|창작|발견|설립|주도|의거|혁명|활동|업적|정책|통치|재위|전쟁|정복|저술|작품)/.test(
          s
        );

      if (isFamilyTmiSentence) return false;

      // 10) 메타 절 제거로 문장이 끊어진 경우 제거
      const isBrokenEnding =
        /(?:으로|로|에서|에게|부터|까지|하며|하고|이고|이며|이자|인|및|또는|그리고|때문에|통해|따라|위해)\s*[.!?]?$/.test(
          s
        );

      if (isBrokenEnding) return false;

      // 11) 정상적인 문장 종결 확인
      const isValidEnding =
        /(?:이다|이었다|였다|있다|없었다|하였다|했다|됐다|되었다|의하여|위함이다|함이다|음이다)\s*[.!?]?$/.test(
          s
        );
