...
export function stripMetainfo(text) {
  ...
  // 반드시 '는', '은', ':', '=' 이 뒤에 바로 붙은 경우만 메타 라벨로 인식
  // (예: '호는', '자는', '본관은', '자:', '호:'만 인식 / '호세', '자유' 등 일반 단어 100% 보호)
  const metaLabelPattern = "(?:(?:본관|시호|아호|별호|아명|태명|세례명|법명|묘호|당호|호|자|묘)(?:는|은|\\s*[:=]))";

  // 1) 괄호 내부 메타 항목 제거 (예: (金九, 1876년~1949년, 호는 백범(白凡)) -> (金九, 1876년~1949년))
  ...
  // 2) 독립된 메타 절/문장 완전 제거 (예: "본관은 현풍(...)으로, 자는 계수(...)이다.")
