...
export function stripMetainfo(text) {
  ...
  for (let sentence of sentences) {
    ...
    // =========================================================
    // 2) 문장 전체가 명백한 메타 정보인지 먼저 검사
    // =========================================================

    const pureMeta =
      /^(?:이름|본관|본적|시호|아호|별호|아명|태명|세례명|법명|묘호|당호|호|자|묘|성|씨|휘)\s*(?:는|은)\s+.+$/;
    ...
    // =========================================================
    // 3) 문장 전체가 순수 가계/혈통 TMI인지 검사
    // =========================================================

    const familyOnly =
      /^(?:(?:그의|그녀의|) ?(?:부친|모친|아버지|어머니|조부|증조부|고조부|외조부|외조모|장인|처남|장남|차남|장녀|차녀|막내).*)$/;

    if (familyOnly.test(s)) {
      continue;
    }
    ...
    const metaClause =
      /(?:^|(?<=[,;]\s))(?:이름|본관|본적|시호|아호|별호|아명|태명|세례명|법명|묘호|당호|자|호|묘|성|씨|휘)\s*(?:는|은)\s+[^.!?]*?(?:등이다|등이었다|이며|이고|이자|이었다|였다|이다|임)(?:\s*,)?/g;

    s = s.replace(metaClause, "");
    ...
    const familyClause =
      /(?:^|(?<=[,;]\s))(?:부친|모친|아버지|어머니|조부|증조부|고조부|외조부|외조모|장인|처남)\s*(?:은|는)\s+[^.!?]*?(?:등이다|등이었다|이며|이고|이자|이었다|였다|이다|임)(?:\s*,)?/g;

    s = s.replace(familyClause, "");
    ...
    // "전국 시대 추나라 사람으로." 같은 메타 제거 찌꺼기
    if (
      /(?:으로|로|에서|에게|부터|까지|하며|이고|이며|이자|및|또는)\s*\.$/.test(
        s
      )
    ) {
      continue;
