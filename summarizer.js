...
export function stripMetainfo(text) {
  ...
  // 1) 괄호 정제 전, 생몰년(년, 월, 일, ~ 포함)은 치환에서 제외하도록 안전 보호
  // 족보/가족 관련 괄호만 핀셋 삭제
  result = result.replace(/\([^)]*(?:부친|모친|조부|증조부|고조부|외가|손자|처남|장인|남씨|윤씨|씨)[^)]*\)/g, "");

  // 2) 출생절 및 파손 조각 제거
  result = result.replace(/(?:의\s*)?(?:첫|둘째|셋째|넷째|다섯째|막내)?\s*(?:번째|째)?\s*(?:아들|딸|남|녀|장남|차남|삼남|사남|오남|장녀|차녀|삼녀|사녀|외아들|외딸)?(?:로|으로)?\s*태어났(?:다|으며|고)|의\s*로\s*태어났(?:다|으며|고)/g, "");

  // 3) 족보/보계/시조/세손/후손 문장절 도려내기
  result = result.replace(/[^,.!?\n]{1,50}?(?:\d+세손|\d+대손|시조|후손)(?:이다|이며|이고|이었다|였다)[,;\s]*/g, "");

  // 4) 혼인/처가/시댁 거주 관련 절 도려내기
  result = result.replace(/(?:[가-힣]+(?:와|과)\s*)?(?:결혼|혼인|성혼|재혼|이혼|파혼)(?:했|하였|되었|됐|하|하고|이후|이후엔)?(?:다|으며|고|자|음|나|엔)?[,;\s]*/g, "");
  result = result.replace(/(?:처가|시댁)(?:인|에서|의)?\s*[^,.!?\n]{1,30}?(?:지냈다|살았다|거주했다)[,;\s]*/g, "");

  // 5) 자녀 명명/형제 이름 나열 절 도려내기
  result = result.replace(/[^,.!?\n]{1,50}?(?:아들|딸|자녀)(?:에게|들의|의)\s*[^,.!?\n]{1,50}?(?:이름|이름자)[^,.!?\n]{1,30}?(?:지었다|붙였다)[,;\s]*/g, "");

  // 6) 메타 라벨(본관, 호, 자 등) 도려내기 (단, 생몰년 날짜 괄호는 건드리지 않음)
  ...
  result = result.replace(/\(([^()]+(?:\([^()]*\)[^()]*)*)\)/g, (match, inner) => {
    // 괄호 안에 연도/날짜(숫자+년/~)가 있으면 생몰년이므로 보존
