...
function filterOtherPerson(rawSentences, aliases = []) {
    ...
    return rawSentences.filter((sentence, index) => {
        ...
        const hasName = safeAliases.some(alias => text.includes(alias));
        const hasPronoun = /(?:^|\s)(?:그는|그가|그의|그를|그녀는|그녀가|이\s*이론은|이\s*기술은)\b/.test(text);
        return hasName || hasPronoun;
    });
}
...
export function extractImportantSentences(bodyText, introText = "", aliases = [], count = 3) {
    ...
    for (let index = 0; index < totalCount; index++) {
        ...
        if (GENERIC_IMPACT_REGEX.test(text)) score += 20;
        if (GENERIC_NOISE_REGEX.test(text)) score -= 35;
        if (len >= 30 && len <= 150) score += 5;
