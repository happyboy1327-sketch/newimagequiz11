...
export function stripMetainfo(text) {
  ...
  // 2) 부모/부인/결혼/이혼 절 도려내기 (탐욕 매칭 범위 제한 {1,30})
  result = result.replace(/(?:부친|모친|조부|증조부|고조부|외조부)\s+[^,.!?\n]{1,30}?(?:낳았고|낳았으며|이었고|이었다|이며|거쳤으나|낙향하였고)[,;\s]*/g, "");
  result = result.replace(/(?:첫|둘째|셋째|두\s*번째)?\s*부인인?\s+[^,.!?\n]{1,30}?(?:사이에|낳았으며|낳았고)[,;\s]*/g, "");
  result = result.replace(/(?:[가-힣]+(?:와|과|과도)\s*)?(?:결혼|혼인|성혼|재혼|이혼|파혼)(?:했|하였|되었|됐|하|하고)(?:다|으며|고|자|음|나)[,;\s]*/g, "");
  result = result.replace(/(?:남편|아내|배우자|처)\s*[가-힣]*(?:와|과|과의|와의)?\s*(?:사이에|결혼|이혼)[,;\s]*/g, "");

  // 3) 개명 / 개칭 / 이칭 / 자녀 출생 절 도려내기
  result = result.replace(/(?:[가-힣]+(?:으)?로\s*)?(?:개명|개칭)(?:했|하였|되었|됐)(?:다|으며|고|자|음)[,;\s]*/g, "");
  result = result.replace(/(?:[가-힣]+(?:이)?라|[가-힣]+(?:으)?로)\s*(?:부르|불리|칭하)기도?\s*(?:했|하였|되었|됐|다|으며|고)[,;\s]*/g, "");
  result = result.replace(/[^,.!?\n]{1,30}?\s*낳았(?:다|으며|고)[,;\s]*/g, "");
  ...
  // 7) [핵심 방어막] 서술어가 잘려나간 불완전한 문장(잘린 조각) 폐기
  const words = result.match(/[가-힣a-zA-Z0-9]{2,}/g) || [];
  if (words.length <= 2) return "";

  // 문장 끝이 '인', '의', '와', '과', '에서', '로' 같은 조사/수식어나 알파벳 단독(H.)으로 끝나면 동사가 잘린 유령 문장이므로 통째로 삭제
  if (/[인과의는은를을에서로으로임함]\s*\.?$/i.test(result) || /[A-Z]\.\s*$/i.test(result)) {
    return "";
  }
