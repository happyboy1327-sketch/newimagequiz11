...
function removeMetaBySearch(text) {
    ...
    // 💡 수정: 마지막 '?'를 제거해 조사(은/는) 또는 괄호가 키워드 바로 뒤에
    // 실제로 붙어있을 때만 매칭되도록 강제. 이전에는 이 부분이 선택적(optional)이라
    // "시호가 전해진다"의 "시호", "호태왕(好太王)"의 "호"처럼 키워드가 단어 일부로
    // 쓰인 경우까지 메타 정보으로 오인해 뒤따르는 텍스트를 통째로 삭제하는 버그가 있었음.
