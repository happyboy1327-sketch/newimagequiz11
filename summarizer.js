...
export function stripMetainfo(text) {
  ...
  const metaKeywords = "자는|호는|시호는|본관은|별호는|아호는|아명은|태명은|세례명은|일명은|당호는|법명은|묘호는|자|호|시호|본관|별호|아호|아명|태명|세례명|일명|당호|법명|묘호";

  // 1) 괄호 내부의 메타 항목 제거 (예: (金九, 1876년~1949년, 호는 백범(白凡)) -> (金九, 1876년~1949년))
  const innerMetaRegex = new RegExp(
    `(?:${metaKeywords})\\s*[:=]?\\s*[^,;)]+(?:\\([^)]*\\))?`,
    "g"
  );
