...
export function stripMetainfo(text) {
  ...
  // 2) 부정형 후방탐색 적용: 앞에 한글 글자가 없는 독립된 '자/호' 메타 라벨만 제거
  // (예: "자는 연하" -> 삭제 / "보호는 연하" -> 유지)
  result = result.replace(/(?<![가-힣])(?:본관|시호|아호|별호|아명|법명|묘호|호|자)\s*[:=는은]\s*[^,;.\n]+/g, "");

  // 3) 문두 인용/출처 수식절 정제
  result = result.replace(/^(?:[^,.!?\n]+?(?:에\s*따르면|에\s*의하면|전기에\s*따르면)[,;\s]*)/gm, "");

  // 4) 다중 공백, 잔여 구두점(,. / , ,) 소탕 및 문장 끝 어미 보정
  result = result
    .replace(/\(\s*\)/g, "")
    .replace(/(?:,\s*)+,/g, ",")          // 연속 쉼표 제거 (, , -> ,)
    .replace(/,\s*\./g, ".")               // 쉼표+마침표 찌꺼기 제거 (, . 또는 ,. -> .)
    .replace(/^\s*,\s*/, "")               // 문두 쉼표 제거
    .replace(/([가-힣]+)(?:으로|며|이고|이자|이며)\s*\./g, "$1이다.") // '정치인으로.' -> '정치인이다.' 완성
    .replace(/\s+/g, " ")
    .trim();
